...
def train_and_evaluate(epochs):
    ...
    while len(list(DEFAULT_EVENT_SUBSETS.keys())) > 0:
        accuracy_classes = {}
        for key, value in DEFAULT_EVENT_SUBSETS.items():
            print(f"Start Trainig class: {key}")
            EVENT_SUBSETS = {}
            EVENT_LABELS = {}
            ...
            EVENT_LABELS[key] = 0 if len(list(EVENT_LABELS.values())) == 0 else max(list(EVENT_LABELS.values())) + 1
            EVENT_SUBSETS[key] = value 
            ...
            #  test_auc_scores.append(test_auc)
                print(f'Epoch: {epoch:03d} , Classe: {key}')
            accuracy_classes[key]= test_accuracies[-1]
            print(accuracy_classes)
        
        key_max_accuracy, value_max_accuracy =  max(accuracy_classes.items(), key=lambda item: item[1])
        EVENT_SELECTED[key_max_accuracy]= DEFAULT_EVENT_SUBSETS[key_max_accuracy]
        EVENT_ACCURACY[key_max_accuracy] = value_max_accuracy
        DEFAULT_EVENT_SUBSETS.pop(key_max_accuracy)
        
        print(f"key_max_accuracy: {key_max_accuracy}")
        print(f"EVENT_SELECTED: {EVENT_SELECTED}")
        print(f"config[\"out_size\"]: {config["out_size"]}")
